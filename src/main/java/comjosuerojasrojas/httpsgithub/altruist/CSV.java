...
public class CSV {
	...
	private InputStream fileName; //is the whole location, if it is not in the same folder
	 ...
	 */
	public CSV(InputStream fileName, String splitBy){
	 ...
	 * @param fileName the location of the file
	 */
	public CSV(InputStream fileName){
		this.fileName = fileName;
		this.splitBy = ",";
		this.numColumns = getNumColumns();
		this.numRows = getNumRows();
	...
	public int getNumColumns(){
		int returnInt = 0;
		Log.d("inputnull", "getNumCol");
		try{
			InputStreamReader a = new InputStreamReader(fileName);
			if(a == null){
				Log.d("inputnull", "");
			}
			buffRead = new BufferedReader(new InputStreamReader(fileName));
	...
	*/
	...
	public String[] readColumn(int column){
		...
		return  returnStringA.split(splitBy);
		}
	...
	public void addRow(String[] input){
		...
		if(input.length == numColumns){
			try {
				...
				buffWrite.append(TextUtils.join(splitBy,input));
				buffWrite.newLine();
